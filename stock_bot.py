...
def check_theme_score(row, vol_col):
    ...
    計算題材分數：
    1. 檢查股名是否包含關鍵字
    2. 使用 yfinance 檢查產業類別 (針對成交量大的前幾名檢查即可，節省時間)
    """
    code = row['代號']
    name = row['名稱']
    vol = row[vol_col]
    ...
    # 1. 基礎分：成交量越大分數越高 (每萬張 +1分)
    score += vol / 10000 
    
    # 2. 股名直接命中 (例如 "華城" 雖沒電字但可能被手動加入，這裡主要抓像 "長榮航" 的 "航")
...
def get_sector_from_yf(code):
    ...
    try:
        ...
        sector = ticker.info.get('sector', '') + ticker.info.get('industry', '')
        # 轉換成中文關鍵字比對 (簡單映射)
        for k in HOT_KEYWORDS:
            # 這裡做簡單的英文對應，或直接看有沒有相關字
            # 實際應用中，yf 回傳多為英文，這裡簡化處理：
            # 若能抓到 News 標題最好
            pass
        
        # 抓新聞標題最準
        if ticker.news:
            news_title = ticker.news[0]['title']
            return news_title
    except:
        pass
    return ""
...
def main():
    ...
    if status != "SUCCESS":
        send_line(f"⚠️ {today} 執行狀況: {status} (詳見 GitHub Logs)")
        return
    ...
    if candidates.empty:
        send_line(f"📊 {today} 策略無符合標的 (量縮)。")
        return

    # 2. 題材加權排序
    # 為了效率，我們先取成交量前 15 名來做「深度題材檢查」
    ...
    for index, row in top_15.iterrows():
        ...
        # 如果股名沒看出題材，加碼查 yfinance 新聞 (耗時，所以只查前15名)
        if not tag:
    ...
    # 發送報告
    msg = f"🔥 【Goodinfo 題材強勢股】 {today}\n"
    msg += "策略：金叉+爆量+站上月線+熱門題材\n\n"
